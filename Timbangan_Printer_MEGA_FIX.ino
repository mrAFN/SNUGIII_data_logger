...
void countMode() {
  ...
  int count = 0;
  int qty = EEPROM.readInt(EE_QTY);
  bool printing = false;
  float datas[qty + 1];
  DateTime now = rtc.now();
  sprintf(lcdBuff, "%02d/%02d/%04d %02d:%02d", now.day(), now.month(), now.year(), now.hour(), now.minute());
  info = String(lcdBuff) + "\r\n" +
         "OPERTATOR  : " + operator_id[EEPROM.readInt(EE_OPERATOR)] + "\r\n" +
         "ITEM       : " + barang_id[EEPROM.readInt(EE_ITEM)] + "\r\n" +
         "BATCH      : " + batch_id[EEPROM.readInt(EE_KODE)]  + "\r\n" +
         "TARGET QTY : " + String(EEPROM.readInt(EE_QTY)) + "\r\n";
  Serial.println(info);
  ...
  while (1) {
    ...
    char buff[11];
    dtostrf(NW, 8, 2, buff);
    sprintf(lcdBuff, "NW:    %sg", buff); // 35040
